...
st.caption(
    "تحليل ذكي متكامل للمواصفات الكهربائية، نوع الجهد، عدد الفازات،"
    " البطاريات، فزة البدء، والتوصيل الميداني"
)
...
def extract_data_via_gemini(panel_img, inverter_img, key):
    ...
    # جلب قائمة الموديلات المتاحة تلقائياً لمنع خطأ 404
    candidate_models = []
    try:
        available_models = client.models.list()
        for m in available_models:
            m_name = m.name.replace("models/", "") if hasattr(m, "name") else ""
            if "flash" in m_name.lower():
                candidate_models.append(m_name)
    except Exception:
        pass

    # إذا لم يستطع جلب القائمة نستخدم الاحتياطية المحدثة
    ...
    for model_name in candidate_models:
        for attempt in range(3):
            try:
                ...
                err_str = str(e)
                last_exception = e
                # الانتقال للموديل التالي إذا كان الموديل غير متوفر 404
                if "404" in err_str or "NOT_FOUND" in err_str:
                    break
                # الانتظار والمحاولة مجدداً في حال تجاوز معدل الطلبات 429
...
if st.button("🔍 تحليل واستخراج التقرير الشامل والحسابات"):
    if not api_key:
        ...
        try:
            ...
            with st.spinner(
                "جاري قراءة كافة بيانات الملصقات وتطبيق الحسابات بـ معاملات"
                " الأمان..."
            ):
                ...
                v_arch = inv.get(
                    "voltage_architecture", "غير موجود على الملصق"
                )
                ...
                strings_per_mppt = safe_int(
                    inv.get("strings_per_mppt"), default=1
                )
                ...
                with col_p_info:
                    ...
                    st.write(
                        f"**الرقم التسلسلي / Part No:** {format_val(p_part)}"
                    )
                    st.write(f"**نوع اللوح:** {format_val(p_type)}")
                    st.write(f"- القدرة (Pmax): {format_val(pmax, 'W')}")
                    st.write(
                        f"- جهد الدارة المفتوحة (Voc): {format_val(voc, 'V')}"
                    )
                    st.write(
                        f"- الجهد التشغيلي (Vmp): {format_val(vmp, 'V')}"
                    )
                ...
                with col_i_info:
                    ...
                    st.write(
                        f"**الرقم التسلسلي / Model No:** {format_val(i_part)}"
                    )
                    st.write(f"**نوع الإنفيرتر:** {format_val(i_type)}")
                    st.write(
                        f"**نظام الفازات (Phase):** {format_val(phase_type)}"
                    )
                    st.write(
                        "**معمارية الجهد (DC Voltage System):**"
                        f" {format_val(v_arch)}"
                    )
                    st.write(
                        "- القدرة المستمرة الاسمية:"
                        f" {format_val(ac_rated_power, 'W')}"
                    )
                    st.write(
                        f"- أقصى جهد مستمر (DC Max): {format_val(v_max, 'V')}"
                    )
                    st.write(
                        f"- أدنى جهد MPPT: {format_val(v_mppt_min, 'V')}"
                    )
                    st.write(
                        f"- أقصى جهد MPPT: {format_val(v_mppt_max, 'V')}"
                    )
                    st.write(
                    ...
                    st.write(
                        "- أقصى تيار لكل MPPT:"
                        f" {format_val(max_mppt_current, 'A')}"
                    )
                ...
                st.subheader(
                    "🔋 مواصفات البطاريات، شبكة AC، وقدرة البدء (Startup)"
                )
                ...
                with c_batt:
                    ...
                    batt_type = batt_info.get(
                        "battery_type", "غير موجود على الملصق"
                    )
                    batt_charge = safe_float(
                        batt_info.get("max_charge_current_a")
                    )

                    if not batt_supported and batt_volts == 0:
                        st.write(
                            "❌ **دعم البطاريات:** `لا يدعم بطاريات (On-Grid /"
                            " Direct Solar)`"
                        )
                    else:
                        ...
                        st.write(
                            "- **جهد البطارية الاسمي:**"
                            f" {format_val(batt_volts, 'V')}"
                        )
                        st.write(
                            "- **أنواع البطاريات المدعومة:**"
                            f" {format_val(batt_type)}"
                        )
                        st.write(
                            "- **أقصى تيار شحن:**"
                            f" {format_val(batt_charge, 'A')}"
                        )

                with c_ac:
                    st.markdown("#### 🔌 مدخل ومخرج AC")
                    ac_v = ac_info.get(
                        "nominal_ac_voltage_v", "غير موجود على الملصق"
                    )
                    ac_freq = ac_info.get(
                        "frequency_hz", "غير موجود على الملصق"
                    )
                    ac_in_curr = safe_float(
                        ac_info.get("max_ac_input_current_a")
                    )
                    ac_out_curr = safe_float(
                        ac_info.get("max_ac_output_current_a")
                    )
                    ...
                    st.write(
                        f"- **أقصى تيار مدخل AC:** {format_val(ac_in_curr, 'A')}"
                    )
                    st.write(
                        f"- **أقصى تيار مخرج AC:** {format_val(ac_out_curr, 'A')}"
                    )
                ...
                with c_surge:
                    ...
                    st.write(
                        f"- **قدرة البدء اللحظية:** {format_val(s_power, 'VA')}"
                    )
                    st.write(
                        "- **مدة التحمل اللحظية:**"
                        f" {format_val(s_duration, 'ثانية')}"
                    )

                if voc == 0 or vmp == 0 or v_max == 0:
                    st.error(
                        "⚠️ لم يتم تعيين كافة القيم الكهربائية الأساسية للجهد"
                        " من الصور (مثل Voc, Vmp, DC Max). يرجى التأكد من"
                        " وضوح الملصقات المحملة."
                    )
                else:
                    v_mppt_min_safe = v_mppt_min * 1.10
                    min_string_safe = (
                        math.ceil(v_mppt_min_safe / vmp) if vmp > 0 else 1
                    )
                    ...
                    max_by_voc = (
                        math.floor(v_max_safe / voc_cold_safe)
                        if voc_cold_safe > 0
                        else 1
                    )
                    max_by_mppt = (
                    ...
                    rec_string = math.floor(
                        (min_string_safe + max_string_safe) / 2
                    )
                    ...
                    if max_mppt_current > 0 and isc_safe > max_mppt_current:
                        st.warning(
                            "⚠️ **تنبيه مطابقة التيار:** تيار القصر المعدل"
                            f" للوح ({round(isc_safe, 2)} A) أكبر من أقصى تيار"
                            " يتحمله مدخل MPPT في الإنفيرتر"
                            f" ({max_mppt_current} A). سيعمل النظام ولكن قد"
                            " يحدث قص للتيار (Clipping) عند الذروة."
                        )
                    elif max_mppt_current > 0:
                        st.success(
                            "✅ **توافق التيار:** تيار اللوح المعدل"
                            f" ({round(isc_safe, 2)} A) متوافق تماماً مع مدخل"
                            f" الإنفيرتر ({max_mppt_current} A)."
                        )
                    ...
                    * **العدد الموصى به مثالياً بالسلسلة:** `{rec_string}` ألواح.
                    ...
                    st.write(
                        "الحدود الكهربائية المسموحة لهذا النظام هي ما بين"
                        f" **{min_total_panels}** إلى **{max_total_panels}**"
                        " لوحاً كإجمالي للمنظومة:"
                    )
                    ...
                    if custom_panels_count > 0:
                        custom_kw = round(
                            (custom_panels_count * pmax) / 1000, 2
                        )
                        st.markdown(
                            "#### 📊 النتائج للعدد المدخل"
                            f" ({custom_panels_count} لوحاً):"
                        )
                        st.write(
                            "- **إجمالي قدرة التوليد (Power):**"
                            f" `{custom_kw} kW` (محسوبة من بقدرة اللوح `{pmax}W`"
                            " من الصورة)"
                        )

                        num_strings_used = min(
                            total_strings, custom_panels_count
                        )
                        panels_per_str = (
                            custom_panels_count // num_strings_used
                        )
                        ...
                        if panels_per_str < min_string_safe:
                            st.error(
                                "❌ **العدد المدخل غير آمن (أقل من الحد"
                                " الأدنى):**\n\nعند توزيع"
                                f" `{custom_panels_count}` لوحاً على السلاسل،"
                                f" سيكون هناك `{panels_per_str}` ألواح"
                                " بالسلسلة الواحدة بجهد تشغيلي قدره"
                                f" `{vmp_string}V`.\n\nوهذا أقل من الحد الأدنى"
                                " للتشغيل الآمن المكتشف من ملصق الإنفيرتر وهو"
                                f" `{min_string_safe}` ألواح (جهد MPPT الأدنى"
                                " معدلاً ="
                                f" `{round(v_mppt_min_safe,1)}V`). لن يعمل"
                                " الإنفيرتر بكفاءة."
                            )
                        elif panels_per_str > max_string_safe:
                            st.error(
                                "⚠️ **العدد المدخل غير آمن (يتجاوز أقصى"
                                f" جهد):**\n\nعند توزيع `{custom_panels_count}`"
                                f" لوحاً، ستحتوي السلسلة على `{panels_per_str}`"
                                " ألواح بجهد دارة مفتوحة في الشتاء يصل إلى"
                                f" `{voc_string_cold}V`.\n\nوهذا يتجاوز الحد"
                                " الأقصى الآمن المسموح به في الإنفيرتر وهو"
                                f" `{max_string_safe}` لوحاً (أقصى جهد مستمر ="
                                f" `{round(v_max_safe,1)}V`). **خطر احتراق"
                                " مدخل الإنفيرتر!**"
                            )
                        else:
                            st.success(
                                "✅ **العدد المدخل متوافق تماماً وآمن"
                                " كهربائياً:**\n\nجهد السلسلة التشغيلي سيكون"
                                f" حوالي `{vmp_string}V` وفي أقصى برودة"
                                f" سيعطي `{voc_string_cold}V`، وكلها تقع ضمن"
                                " نطاق أمان الإنفيرتر المكتشف من الصورتين."
                            )
                            ...
                            * **الجهد الأقصى المتوقع في الشتاء (Voc Cold):** `{voc_string_cold} V`
                            """
                                + (
                                    "\n⚠️ **ملاحظة:** يتبقى"
                                    f" `{remainder}` ألواح غير موزعة. لضمان"
                                    " اتزان الجهد بين السلاسل، يفضل أن يكون"
                                    " إجمالي عدد الألواح يقبل القسمة بالتساوي"
                                    " على عدد السلاسل المستخدمة."
                                    if remainder > 0
                                    else ""
                                )
                            )
            ...
            if "429" in err_msg or "RESOURCE_EXHAUSTED" in err_msg:
                st.error(
                    "⏳ **تم تجاوز عدد الطلبات المسموح بها مؤقتاً (Rate"
                    " Limit).** يرجى الانتظار بضع ثوانٍ وإعادة المحاولة."
                )
            else:
