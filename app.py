...
# 2. تخصيص الواجهة وتدفق النصوص (RTL) مع تحسين التصميم البصري
st.markdown(
    """
    <style>
...
with st.sidebar:
    ...
    st.info("💡 المفتاح مطلوب لعمليات التحليل واستخراج البيانات من النماذج.")
    
    st.markdown("---")
    st.markdown("### 📌 معلومات الاستخدام")
    st.markdown("تأكد من إدخال بيانات صحيحة أو صور واضحة لملصقات الأجهزة للحصول على نتائج دقيقة وموثوقة.")
...
if "📸" in search_mode:
    ...
    with cols[0]:
        uploaded_panel = st.file_uploader(
            "📸 صورة ملصق اللوح الشمسي", type=["jpg", "jpeg", "png", "webp"]
        )
    with cols[1]:
        uploaded_inverter = st.file_uploader(
            "📸 صورة ملصق الإنفيرتر", type=["jpg", "jpeg", "png", "webp"]
        )
    if enable_battery:
        with cols[2]:
            uploaded_battery = st.file_uploader(
                "📸 صورة ملصق البطارية", type=["jpg", "jpeg", "png", "webp"]
            )
else:
    cols = st.columns(3 if enable_battery else 2)
    with cols[0]:
    ...
    with cols[1]:
        inverter_text_query = st.text_input(
            ...
            placeholder="مثال: Deye SUN-5K-SG04LP1-EU",
        )
    if enable_battery:
        with cols[2]:
            battery_text_query = st.text_input(
                "🔋 اسم الشركة والموديل للبطارية:",
                placeholder="مثال: Pylontech US3000C",
            )
...
# 6. دالة الاستخراج عن طريق الصور مع نظام إعادة المحاولة
def extract_via_images(panel_img, inverter_img, battery_img, key):
    ...
    prompt = f"""
    أنت مهندس طاقة شمسية خبير. قم بتحليل الصور المرفقة واستخرج البيانات التالية بدقة بصيغة JSON فقط دون أي نصوص إضافية أو مقدمات:
    {JSON_STRUCTURE}
    ملاحظة:
    - أعد أرقاماً فقط للقيم الرقمية، واستخدم 0 للقيم المفقودة.
    ...
    for attempt in range(max_retries):
        try:
            response = client.models.generate_content(
                model="models/gemini-2.5-flash",  # تم تحديث الموديل ليكون مستقراً وسريعاً
                contents=contents,
...
# 7. دالة الاستخراج عن طريق اسم الموديل (نصياً) مع نظام إعادة المحاولة
def extract_via_text(p_text, i_text, b_text, key):
    ...
    prompt = f"""
    أنت خبير بقواعد بيانات كتالوجات الألواح الشمسية والإنفيرترات والبطاريات (Datasheets).
    اللوح الشمسي المطلوب: "{p_text}"
    الإنفيرتر المطلوب: "{i_text}"
    {b_prompt}

    استخرج المواصفات القياسية لهذه الموديلات وعد بتقرير بصيغة JSON بنفس الهيكل تماماً بدون أي مقدمات:
    {JSON_STRUCTURE}

    تنبيه هام:
    - أعد أرقاماً فقط للقيم الرقمية.
    - التزم بدقة المواصفات الفنية الحقيقية (Datasheet).
        ...
        if "📸" in search_mode:
            if not uploaded_panel or not uploaded_inverter:
                ...
                try:
                    ...
                    st.error(f"حدث خطأ أثناء معالجة الصور أو الاتصال بالخادم: {e}")
        else:
...
if "analysis_result" in st.session_state and st.session_state["analysis_result"]:
    ...
    if is_on_grid and has_external_battery:
        system_errors.append(
            "⚠️ **تناقض خطير:** لقد قمت بإدخال/تفعيل بطارية خارجية مع إنفيرتر شبكي (On-Grid). إنفيرترات On-Grid التقليدية لا تدعم توصيل البطاريات بشكل مباشر."
        )
        ...
        * **العدد الموصى به مثالياً بالسلسلة:** `{rec_string}` ألواح.
        ...
        if custom_panels_count > 0:
            ...
            if panels_per_str < min_string_safe:
                ...
                distribution_msg = f"🔌 **خطة التوصيل:** استخدم `{num_strings_used}` سلاسل، وكل سلسلة تضم `{panels_per_str}` ألواح."
                if remainder > 0:
                    distribution_msg += f" (يوجد باقي {remainder} ألواح يجب تووزيعها بعناية أو تعديل العدد الإجمالي)."
                st.info(distribution_msg)
