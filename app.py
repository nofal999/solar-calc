...
# متغيرات الإدخال اليدوي الكامل
m_pmax, m_voc, m_vmp, m_isc, m_imp = 550.0, 49.6, 41.5, 14.0, 13.2
m_ac_power, m_v_max, m_v_mppt_min, m_v_mppt_max, m_mppt_count, m_max_mppt_curr = 5000.0, 550.0, 125.0, 500.0, 2, 18.0
m_inv_type = "Hybrid"
    ...
    if "🔢" in search_mode:
        res = {
            ...
            "inverter": {
                ...
                "strings_per_mppt": 1, "max_mppt_current": m_max_mppt_curr,
                "battery": {"supported": m_inv_type != "On-Grid", "nominal_voltage_v": m_b_volts if enable_battery else 48.0},
                "ac_input_output": {"nominal_ac_voltage_v": "220V", "frequency_hz": "50Hz"},
...
if "analysis_result" in st.session_state and st.session_state["analysis_result"]:
    ...
    with col_i_info:
        ...
        st.write(f"- مداخل MPPT: `{mppt_count}` (أقصى تيار `{max_mppt_current}A`)")
        ...
        * **العدد الموصى به مثالياً بالسلسلة:** `{rec_string}` ألواح.
        ...
        if custom_panels_count > 0:
            ...
            if panels_per_str < min_string_safe:
                ...
                if remainder > 0: dist_msg += f" (ملاحظة: يوجد باقي {remainder} ألواح)."
                st.info(dist_msg)
