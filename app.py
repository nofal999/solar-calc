...
st.caption(
    ...
    " التحكم بمداخل الـ MPPT، وتوزيع السلاسل الميدانية آلياً"
)
...
if "📸" in search_mode:
    ...
    st.markdown("---")
    st.subheader("🔢 إدخال القيم الفنية والتحكم بمداخل MPPT يدوياً")
...
JSON_STRUCTURE = """
{
  ...
  "inverter": {
    "brand": "...", "model": "...", "type": "...", "phase_type": "Single-Phase",
    "ac_rated_power_w": 0.0, "v_max": 0.0, "v_mppt_min": 0.0, "v_mppt_max": 0.0,
    "mppt_count": 1, "strings_per_mppt": 1, "max_mppt_current": 0.0,
    "battery": {"supported": true, "nominal_voltage_v": 0.0}
  },
  "external_battery": {"brand": "...", "model": "...", "chemistry": "...", "capacity_ah": 0.0, "capacity_kwh": 0.0, "nominal_voltage_v": 0.0}
}
"""
...
def extract_via_images(panel_img, inverter_img, battery_img, key):
    ...
    if battery_img: contents.append(compress_image_for_speed(battery_img))
    prompt = f"استخرج البيانات بدقة بصيغة JSON فقط حسب هذا الهيكل:\n{JSON_STRUCTURE}"
...
def extract_via_text(p_text, i_text, b_text, key):
    client = genai.Client(api_key=key)
    prompt = f"اللوح: '{p_text}', الإنفيرتر: '{i_text}', البطارية: '{b_text}'. استخرج بصيغة JSON فقط حسب الهيكل:\n{JSON_STRUCTURE}"
    response = client.models.generate_content(
...
if st.button(trigger_label):
    ...
    if "🔢" in search_mode:
        res = {
            "panel": {"brand": "إدخال يدوي", "model": "مخصص", "pmax": m_pmax, "voc": m_voc, "vmp": m_vmp, "isc": m_isc, "imp": m_imp},
            "inverter": {
                ...
                "v_mppt_max": m_v_mppt_max, "mppt_count": int(m_mppt_count), "strings_per_mppt": int(m_strings_per_mppt), 
...
# 7. عرض النتائج وتحليل مداخل MPPT
if "analysis_result" in st.session_state and st.session_state["analysis_result"]:
    ...
    if max_mppt_current > 0 and isc_safe > max_mppt_current:
        st.warning(f"⚠️ **تحذير تيار:** تيار اللوح المعدل ({round(isc_safe, 2)}A) أعلى من أقصى تيار لمدخل MPPT ({max_mppt_current}A).")

    st.markdown("---")
    st.subheader("📌 ملخص المواصفات وتحكم مداخل الـ MPPT")
    ...
    with col2:
        ...
        st.write(f"- **عدد مداخل MPPT:** `{mppt_count}`")
        st.write(f"- **السلاسل لكل مدخل:** `{strings_per_mppt}`")
    ...
    if voc > 0 and vmp > 0 and v_max > 0:
        ...
        * **إجمالي السلاسل المتاحة على الإنفيرتر:** `{total_strings}` سلاسل موزعة على `{mppt_count}` مداخل.
        """)
